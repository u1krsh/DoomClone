...
void draw2d() {
	...
	for (s = 0; s < numSect; s++) {
		for (w = S[s].ws; w < S[s].we; w++) {
			if (s == G.selS - 1) {
				...
				drawPx(W[w].x1 / G.scale, W[w].y1 / G.scale, 255, 255, 255);
				drawPx(W[w].x2 / G.scale, W[w].y2 / G.scale, 255, 255, 255);
	...
	int dx = M.sin[P.a] * 12;
	int dy = M.cos[P.a] * 12;
	drawPx(P.x / G.scale, P.y / G.scale, 0, 255, 0);
	drawPx((P.x + dx) / G.scale, (P.y + dy) / G.scale, 0, 175, 0);
...
void darken() {
	int x, y, xs, xe, ys, ye;
	if (dark == 0) { return; } //no button were clicked
	...
	if (dark == 5) { xs = 11; xe = 11; ys = 96 / G.scale; ye = 128 / G.scale; }
	...
	if (dark == 9) { xs = 07; xe = 15; ys = 224 / G.scale; ye = 256 / G.scale; }
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		else
		{
			...
			else if (G.addSect == 3)
			{
				...
				ang = (ang * 180) / 3.14159265358979323846;      //radians to degrees
...
void mouseMoving(int x, int y) {
	if (x<580 && G.addSect == 0 && G.move[0]>-1) {
		int Aw = G.move[0], Ax = G.move[0] > -1;
		int Bw = G.move[2], Bx = G.move[3];
		if (Ax == 1) { W[Aw].x1 = ((x + 16) >> 5) << 5; W[Aw].y1 = ((GSLH - y + 16) >> 5) << 5; }
		if (Ax == 2) { W[Aw].x2 = ((x + 16) >> 5) << 5; W[Aw].y2 = ((GSLH - y + 16) >> 5) << 5; }
		if (Bx == 1) { W[Bw].x1 = ((x + 16) >> 5) << 5; W[Bw].y1 = ((GSLH - y + 16) >> 5) << 5; }
		if (Bx == 2) { W[Bw].x2 = ((x + 16) >> 5) << 5; W[Bw].y2 = ((GSLH - y + 16) >> 5) << 5; }
	}
}
...
void display() {
	...
	if (T.fr1 - T.fr2 > 50)
	{
		...
		T.fr2 = T.fr1;
		glutSwapBuffers();
		glutReshapeWindow(GSLW, GSLH); // prevents window scaling
	}
...
void init() {
	...
	for (x = 0; x < 360; x++) {
		M.cos[x] = cos(x / 180.0 * 3.14159265358979323846);
		M.sin[x] = sin(x / 180.0 * 3.14159265358979323846);
...
int main(int argc, char* argv[])
{
	...
	gluOrtho2D(0, GSLW, 0, GSLH);                      //origin bottom left
	init();
